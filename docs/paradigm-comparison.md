...
### 成立：AI 产配置能力（交付物 A · Q-003 · 2026-07-03）

**AFP 的核心叙事之一是「AI 产配置、引擎确定执行」。这一维度已由 DeepSeek 单模型跨任务实测建立底线证据**——见 [`docs/agent-test-prompts.md`](./agent-test-prompts.md) 「已回填记录 · A-01 / B-01 / C-01」。

| 任务类型 | 提示词 | 结论 | 关键证据 |
| :--- | :--- | :--- | :--- |
| 加一关（创作） | A-01 · L 形走廊 8×8 | ✅ 通过 | 格式纪律满分、CLI check 通过、8×8 尺寸对、开局非通关 |
| 加新地形（设计） | B-01 · 冰面（含追问一轮） | ★★★☆☆ 通过底线 | 硬边界纪律 5/5 通过；能主动识别「数据加载层的纯机械映射不属算法」、追问后能修正立场 |
| 修 bug（诊断） | C-01 · 畸形关 boundary-not-closed | ✅ 通过 | 0-indexed 映射准确、flood-fill 解释正确、修复极简不误改 |

**四条错误 pattern**（首次登记，等待跨模型稳定性验证）**集中在「设计质量 / 产品级考量」维度，而非「纪律遵守」维度**——这是本轮实测最重要的方向性结论。
...
**意义**：AI 能守 AFP 纪律（硬边界 5/5 通过）—— 这是"AI 时代范式"卖点得以成立的必要条件。剩下的错误 pattern 是**设计品味 / 产品级质量**层面，不是**范式合规**层面——**AFP 结构约束下 AI 的自由度收窄，反而更容易产出正确结果**。这条结论支撑 Studio 方向的 AI 交互设计（AI 帮加关卡 / 加地形 / 修 bug 三类任务已建立能力底线）。
...
| :--- | :--- | :--- | :--- |
| （MVP-1/2/3 + 交付物 A 均未暴露新崩点；剩余证据缺口 = Q-001 演化 / 场景多样性；MVP-4 撤销机制的边界证据已归档，见 D-015） | — | — | — |
...
## 结论（Sokoban 链收官 + 交付物 A 完成时的阶段性总结）

**Sokoban 场景下已建立的正面证据（三条）**：

1. **AFP 数据流在核心玩法（走路 + 推箱 + 胜利判定）场景下成立**——纯块 + 配置 + 方案 A 全量穿透覆盖了完整回合语义。
2. **AFP 不承担回合控制流是合理边界**——跨回合的时间维度状态 + 事件级条件分支由浏览器侧命令式代码承接（MVP-2 唯一一处 `@paradigm NON-AFP` 标记），这是正面证据而非失败。
3. **AI 能在 AFP 结构下产出合规配置**（交付物 A · Q-003 · DeepSeek 单模型跨任务）——硬边界纪律 5/5 通过，错误集中在"设计品味"而非"纪律违规"。

**未闭合的两块缺口**：
- Q-001（演化可靠性）—— 项目最大诚实债，零实证
- 场景多样性 —— 正面证据集中在 Sokoban 一个场景，易被质疑过拟合

**下一章候选方向**：
- **第二场景验证**（如 RPG Maker 风格角色动画 / 人物生成器）—— 消灭场景多样性空档 + Studio 产品化预演
- **Q-001 演化实证** —— 偿还项目最大诚实债

该结论若被后续场景推翻（例如某场景下 AFP 数据流表达不了 + AI 也产不出合规配置），将在此文档如实更新。
