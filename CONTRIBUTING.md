...
### 🟢 AI 自测复跑（独家价值 · 交付物 A 已发布，跨模型证据加固进行中）

[交付物 A：AI 介入提示词自测包](docs/agent-test-prompts.md) 已发布。**基线已建立**（2026-07-03 · DeepSeek 单模型跨任务 · Q-003 已 verified · 三类任务 A/B/C 全部通过底线）。现在最需要的是**跨模型复跑**——同一版上下文包 + 同一条提示词，看错误 pattern 是否跨模型稳定复现。

可参与的方式：
- 打开 [`docs/agent-test-prompts.md`](docs/agent-test-prompts.md)，挑一条任务（A 加一关 / B 加新地形 / C 修 bug）
- 粘贴 [`docs/agent-test-context.md`](docs/agent-test-context.md) 全文 + 提示词正文到你常用的大模型（Claude / ChatGPT / Gemini / 其它 DeepSeek 版本等）
- 用文档里的「观察点」清单打钩，把结果按「记录模板」形式以 Issue / Discussion / PR 反馈
- 不需要你懂 AFP 所有细节，跟着模板走即可

**特别欢迎的场景**：
- **重现四条已登记的错误 pattern**（契约完备性局部化 / 无产品级考量 / 主题表面化 / 零内部障碍偏保守）——3 家模型复现 → 触发上下文包改进
- **发现新 pattern**——即使模型跑通了，如果你观察到本清单没有的错误特征，也是关键反馈
- **反例**：如果某个模型**跑不出合规产出**（违反硬边界），那是 Q-003 的负面证据，同等有价值

**为什么这一档独家**：跨模型证据只能由外部提供——维护者一个人跑所有模型既不现实也不客观。你用什么模型都算贡献。
