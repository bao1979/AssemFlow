...
- [x] 1. 搭实验骨架（package.json / tsconfig，沿用 exp01 约定）
...
- [x] 2. 实现纯转移核 `src/traffic-light.ts`
...
- [x] 4. 块包装与注册 `src/blocks/register.ts`
...
- [x] 5. 配置 `src/configs/`（A/B 各一份，差异即对比材料）
...
- [x] 6. 方案 A 驱动 `src/driver-a.ts`（纯 AFP）
...
- [x] 7. 方案 B 驱动 `src/driver-b.ts`（@paradigm 有状态块）
...
- [x] 8. A/B 行为等价测试 `tests/ab-equivalence.test.ts`
...
- [x] 10. 对比报告 `REPORT.md`（Requirement 2 核心交付物）
